/*
 For node K which has Temperature sensor 
...
uint8_t temp_data = 0 , soil_data = 0 , light_data = 0, nst= 0,ned = 20;
int state = 0;
...
void read_send(){
 ...
 byte tempValue = analogRead(A0);
 tempValue = tempValue * 0.48828125;
  
 Serial.print("temp value = ");
 Serial.println(temp_data);
   Serial.print(" post temp value = ");
   tempValue = (tempValue + temp_data)/2; 
  temp_data = tempValue;
...
//7E 00 0D 00 01 00 13 A2 00 41 63 0C 49 00 01 12 48    f7
 int b= 0x192 + temp_data;// + 0x01;// {
 int b1 = 0x193 + soil_data; //+ 0x02;
 int b2 = 0x194 + light_data; //+ 0x03;
 
// 7E 00 0D 00 01 00 13 A2 00 41 62 FF FE 00 01 12 4C
 int c= 0x1D4 + temp_data ;//+ 0x01;
 int c1 = 0x1D5 + soil_data; // + 0x02;
 int c2 = 0x1D6 + light_data ; //+ 0x03;

//7E 00 0D 00 01 00 13 A2 00 41 63 0C 3C 

  int d= 0x19D + temp_data ;//+ 0x01;
 int d1 = 0x19E + soil_data; // + 0x02;
 int d2 = 0x19F + light_data ; //+ 0x03;
...
// ------------------------------------------------------------------ Frames for node G -------------------------------------------------------------
//7E 00 0D 00 01 00 13 A2 00 41 63 0C 2B 00 01 12 48
byte end1core[] = {0x7E,0x00,0x0D,0x00,0x01,0x00,0x13,0xA2,0x00,0x41,0x63,0x0C,0x2B,0x00,0x01,temp_data,checksumb};
byte end1guest1[] = {0x7E,0x00,0x0D,0x00,0x01,0x00,0x13,0xA2,0x00,0x41,0x63,0x0C,0x2B,0x00,0x02,soil_data,checksumb1};
byte end1guest2[] = {0x7E,0x00,0x0D,0x00,0x01,0x00,0x13,0xA2,0x00,0x41,0x63,0x0C,0x2B,0x00,0x03,light_data,checksumb2};

//-------------------------------------------------------------------Frames for node E --------------------------------------------------------------
 // 7E 00 0D 00 01 00 13 A2 00 41 63 0C 3C 00 01 12 4C
 byte end2core[] = {0x7E,0x00,0x0D,0x00,0x01,0x00,0x13,0xA2,0x00,0x41,0x63,0x0C,0x6D,0x00,0x01,temp_data,checksumc}; 
 byte end2guest1[] = {0x7E,0x00,0x0D,0x00,0x01,0x00,0x13,0xA2,0x00,0x41,0x63,0x0C,0x6D,0x00,0x02,soil_data,checksumc1};
 byte end2guest2[] = {0x7E,0x00,0x0D,0x00,0x01,0x00,0x13,0xA2,0x00,0x41,0x63,0x0C,0x6D,0x00,0x03,light_data,checksumc2}; 
 
//-------------------------------------------------------------------Frames for Sink --------------------------------------------------------------
 // 7E 00 0F 10 01 00 13 A2 00 41 63 0C 49 FF FE 00 00 F3 50
 //7E 00 0D 00 01 00 13 A2 00 41 63 0C 3F 
 byte end3core[] = {0x7E,0x00,0x0D,0x00,0x01,0x00,0x13,0xA2,0x00,0x41,0x63,0x0C,0x36,0x00,0x01,temp_data,checksumd}; 
 byte end3guest1[] = {0x7E,0x00,0x0D,0x00,0x01,0x00,0x13,0xA2,0x00,0x41,0x63,0x0C,0x36,0x00,0x02,soil_data,checksumd1};
 byte end3guest2[] = {0x7E,0x00,0x0D,0x00,0x01,0x00,0x13,0xA2,0x00,0x41,0x63,0x0C,0x36,0x00,0x03,light_data,checksumd2}; 
