/*
 For node D which has light intensity sensor 
...
//  int val = map(LDRValue,0,1023,0,10);
//  Serial.println(val);
//  delay(1000);
...
//7E 00 0D 00 01 00 13 A2 00 41 63 0C 3C 00 02 12 46    f7
 int b= 0x1D9 + temp_data;// + 0x01;// {
 int b1 = 0x1DA + soil_data; //+ 0x02;
 int b2 = 0x1DB + light_data; //+ 0x03;
 
// 7E 00 0D 00 01 00 13 A2 00 41 63 0C 2F FE 00 01 12 49
 int c= 0x344 + temp_data ;//+ 0x01;
 int c1 = 0x345 + soil_data; // + 0x02;
 int c2 = 0x346 + light_data ; //+ 0x03;

//7E 00 0D 00 01 00 13 A2 00 41 63 0C 6D 00 01 12 3D

 int d= 0x1A5 + temp_data ;//+ 0x01;
 int d1 = 0x1A6 + soil_data; // + 0x02;
 int d2 = 0x1A7 + light_data ; //+ 0x03;
...
byte end1core[] = {0x7E,0x00,0x0D,0x00,0x01,0x00,0x13,0xA2,0x00,0x41,0x63,0x0C,0x72,0x00,0x01,temp_data,checksumb};
byte end1guest1[] = {0x7E,0x00,0x0D,0x00,0x01,0x00,0x13,0xA2,0x00,0x41,0x63,0x0C,0x72,0x00,0x02,soil_data,checksumb1};
byte end1guest2[] = {0x7E,0x00,0x0D,0x00,0x01,0x00,0x13,0xA2,0x00,0x41,0x63,0x0C,0x72,0x00,0x03,light_data,checksumb2};

//-------------------------------------------------------------------Frames for node O --------------------------------------------------------------
 // 7E 00 0F 10 01 00 13 A2 00 41 63 0C 49 FF FE 00 00 F3 50
 //7E 00 0D 00 01 00 13 A2 00 41 63 0C 2F 00 01 12 49
 byte end2core[] = {0x7E,0x00,0x0D,0x00,0x01,0x00,0x13,0xA2,0x00,0x41,0x62,0xFE,0xEC,0x00,0x01,temp_data,checksumc}; 
 byte end2guest1[] = {0x7E,0x00,0x0D,0x00,0x01,0x00,0x13,0xA2,0x00,0x41,0x62,0xFE,0xEC,0x00,0x02,soil_data,checksumc1};
 byte end2guest2[] = {0x7E,0x00,0x0D,0x00,0x01,0x00,0x13,0xA2,0x00,0x41,0x62,0xFE,0xEC,0x00,0x03,light_data,checksumc2}; 
 
//-------------------------------------------------------------------Frames for node F --------------------------------------------------------------
 // 7E 00 0F 10 01 00 13 A2 00 41 63 0C 49 FF FE 00 00 F3 50
 //7E 00 0D 00 01 00 13 A2 00 41 63 0C 6D 00 01 12 3D
 byte end3core[] = {0x7E,0x00,0x0D,0x00,0x01,0x00,0x13,0xA2,0x00,0x41,0x63,0x0C,0x3E,0x00,0x01,temp_data,checksumd}; 
 byte end3guest1[] = {0x7E,0x00,0x0D,0x00,0x01,0x00,0x13,0xA2,0x00,0x41,0x63,0x0C,0x3E,0x00,0x02,soil_data,checksumd1};
 byte end3guest2[] = {0x7E,0x00,0x0D,0x00,0x01,0x00,0x13,0xA2,0x00,0x41,0x63,0x0C,0x3E,0x00,0x03,light_data,checksumd2}; 
