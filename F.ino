/*
 For node F which has SOil sensor 
...
//  float soil_per = float(val) * float(100);
 ...
 Serial.println(soil_data);
   Serial.print(" post soil value = ");
   soilValue = (soil + soil_data)/2; 
  soil_data = soilValue;
...
//7E 00 0D 00 01 00 13 A2 00 41 63 0C 3E 00 01 12 48    f7
...
// 7E 00 0D 00 01 00 13 A2 00 41 62 FF FE 00 01 12 4C
 int c= 0x345 + temp_data ;//+ 0x01;
 int c1 = 0x346 + soil_data; // + 0x02;
 int c2 = 0x347 + light_data ; //+ 0x03;

//7E 00 0D 00 01 00 13 A2 00 41 63 0C 3C 

 int d= 0x1A4 + temp_data ;//+ 0x01;
 int d1 = 0x1A5 + soil_data; // + 0x02;
 int d2 = 0x1A6 + light_data ; //+ 0x03;
...
//-------------------------------------------------------------------Frames for node O --------------------------------------------------------------
 // 7E 00 0D 00 01 00 13 A2 00 41 63 0C 2F 00 01 12 4C
 byte end2core[] = {0x7E,0x00,0x0D,0x00,0x01,0x00,0x13,0xA2,0x00,0x41,0x62,0xFF,0xEC,0x00,0x01,temp_data,checksumc}; 
 byte end2guest1[] = {0x7E,0x00,0x0D,0x00,0x01,0x00,0x13,0xA2,0x00,0x41,0x62,0xFF,0xEC,0x00,0x02,soil_data,checksumc1};
 byte end2guest2[] = {0x7E,0x00,0x0D,0x00,0x01,0x00,0x13,0xA2,0x00,0x41,0x62,0xFF,0xEC,0x00,0x03,light_data,checksumc2}; 
 
//-------------------------------------------------------------------Frames for node C --------------------------------------------------------------
 // 7E 00 0F 10 01 00 13 A2 00 41 63 0C 49 FF FE 00 00 F3 50
 //7E 00 0D 00 01 00 13 A2 00 41 63 0C 44 
 byte end3core[] = {0x7E,0x00,0x0D,0x00,0x01,0x00,0x13,0xA2,0x00,0x41,0x63,0x0C,0x3D,0x00,0x01,temp_data,checksumd}; 
 byte end3guest1[] = {0x7E,0x00,0x0D,0x00,0x01,0x00,0x13,0xA2,0x00,0x41,0x63,0x0C,0x3D,0x00,0x02,soil_data,checksumd1};
 byte end3guest2[] = {0x7E,0x00,0x0D,0x00,0x01,0x00,0x13,0xA2,0x00,0x41,0x63,0x0C,0x3D,0x00,0x03,light_data,checksumd2}; 
